...
- [x] **9. The demo draws, first cut.** `demos/tangent.ts` carrying the grid, the axes and the curve;
      `demos/render.ts` turning a figure into SVG text; `npm run demos` writing `docs/tangent.svg`;
      a gate comparing the regenerated bytes against the committed file; and the README carrying the
      image. **Landed.** The demo is 70 marks at every time, 42 of grid, 15 of x axis, 12 of y axis
      and the curve, and `docs/tangent.svg` is 13,828 bytes. Nothing moves yet, so the marks at 0, at
      the still time and at 4 seconds are the same list. The demos are emitted by a third tsconfig
