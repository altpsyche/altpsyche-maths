...
is left, since 2.1.0 through 2.10.0 are cut.

| version | what lands | what it changes | steps | cut against | depends on | plan |
| --- | --- | --- | --- | --- | --- | --- |

**No version is on the ladder.** 4.0.0 was the last row and it is cut, recorded under Now.
...
still waits on outlines** is a plain label on a card, which is 2.10.0, and a column width measured
rather than given.
...
device either, where a curve drawn as a curve needs a winding the engine cannot count.
...
**4.0.0 is cut and not published, and its nine done-criteria are verified here line by line.** The
five `item` names are `entry`, the format is version 2, and a figure is read with held values: an
input is a track a reader may hold, and a pointer moves it `along` a path, by a `drag` in a
direction, or `around` a centre. The flat demo's dot is dragged along its curve and the solid demo's
eye is turned by a drag.

1. No source, test, demo or specification file names `SpaceItem`, `SpaceItemRecord`,
   `SceneItemRecord`, or an `items` parameter or key of `scene3`. The one file outside this one that
   names them is the README's Moving from 3.x, whose table lists them as the names 4.0.0 replaced.
2. SPECIFICATION.md line 1 reads version 2. Its version section and its `scene3` row changed in
   step 2 with the checker, and `inputs` with its three kinds in step 6 with the checker.
3. A version 1 file is refused as "this reads version 2 of the format and the file is written in
   version 1", which is step 2's reading.
4. The eight committed files read at version 2 to the counts they read at version 1, at the still
   and at half the duration, from boolean's 12 and 12 to tangent's 236 and 244, which is step 2's
   reading.
5. `npm run demos` changed 0 bytes of the images at each of steps 1 to 8 and at the cut. Steps 7
   and 8 grew the two figure files by their `inputs`, 410,756 to 411,913 and 275,402 to 275,590.
6. With `s` held at what its track reads, the flat demo at its 7 named times and the solid demo at
   its 4 draw the marks the track gives within 1e-9. `turn` held at 0.5 at 10.5 seconds draws the
   marks of 9.3 seconds within 1e-9.
7. A press on the dot and 0.25 units off it takes `s` with a reach of 0.2, where 0.5 off takes
   nothing. A pointer on the curve at x = 1.5 holds `s` at 0.42815 and the dot drawn with it sits
   2.4e-14 from the pointer. A drag of 8.2 units across the solid demo from a press at 0.25 holds
   `turn` at 1.25 within 1e-12, which is one turn for the width of the frame.
8. `recordFigure` and `framesOf` take no held values, and `gate:record` writes 34 of 34
   recordings at the cut.
9. `package.json` reads 4.0.0 and nothing is published. `npm test` passes 1,556 of 1,556 over 100
   files, `type-check` and `build` report no error, and `check:vocab` reads 0 and 0.

The README and the guide each gained an Inputs section, and the README's Moving from 3.x lists the
five renames and the format change. REFERENCE.md stated inputs from step 4 on.

**What the site owes at the crossing**, from `/home/siva/dev/altpsyche-dev`:

- `components/figure/FigureSurface.tsx` gains the listeners. A press turns its pixel into a place by
  `placeAt` and takes an input by `inputAt` over the marks it drew. A move holds the input's track at
...
beside the rectangle `clip`, and every painter draws it. Both demos' lenses are cut to a rectangle
with corners rounded by 0.12.

1. `docs/SPECIFICATION.md` states `clipPath` at lines 141 to 148, with its units, its rule and its
   nesting, and it changed in 5752d68, the same commit as the checker that reads it.
2. SPECIFICATION.md line 1 reads version 1. Step 1 wrote a file with a `clipPath` back to the same
   bytes, and step 2 read the eight committed files to the same mark counts at the still and at half
   the duration, from 12 on the boolean demo to 1,070 on the solids.
3. A mark with both clips is drawn inside both: the SVG painter writes the rectangle's clip on a `<g>`
   round the path's, the canvas painter records `rect`, `clip`, then the path and `clip` with
   `nonzero`, and the GPU frame cuts by `clipTriangles` and then `clipTrianglesToPath`.
4. Two discs of radius 1 moved 1 apart nest to one path of 4 cubics enclosing 1.22887 against
   2π/3 − √3/2 = 1.22837, inside the allowance of 1.76 × 10⁻³.
5. Steps 3 and 4 changed 0 bytes of the committed images. Step 7 changed the tangent and surface
   images and their strips, which are the two figures that gained a path clip, and no other file,
   and `npm run demos` at the cut changes 0 bytes.
6. `npm run gate:gpu` reads 8 of 8 figures within 8 of 255 over 96% of their pixels with labels and
   97% without, on WebGL 2 on this machine. Tangent reads 91.10% equal and worst 228, surface 88.52%
   and 228 with labels, and the WebGPU loss reports `destroyed` after 0.1ms. `npm run gate:record`
   writes 34 of 34 recordings.
7. The point 0.01 in from each lens's lower left corner is outside the drawn lens in both demos, and
   every one of the 40 inset marks of the flat demo and the 75 of the solid demo carries a
   `clipPath`.
8. `package.json` reads 3.4.0, and nothing is published. `npm test` passes 1,536 of 1,536 over 98
   files, `type-check` and `build` report no error, and `check:vocab` reads 0 and 0.

The header of `figure/mark.ts`, the Restrictions of the README and the guide, and the guide's Clips
and insets say a clip may be a path.
...
comment in the 83 published sources, and `npm run check:vocab` exits 1 on a banned noun or a banned
voice pattern. It reads 0 and 0 at the cut. Stripping comments with the TypeScript printer leaves
all 83 sources identical from `c00297f` to `352e698` and from `78e7876` to the cut, and the
local renames in `78e7876` leave all 83 `dist/*.d.ts` identical. `npm test` passes 1481 of 1481.
...
subset to the 205 glyphs a figure writes, 15,188 bytes carried as a base64 module behind a dynamic
import, under the SIL Open Font License 1.1. `readFont` reads the TrueType tables, `glyphPath` gives a
glyph's contours as this package's own cubic path, and `textOutlines` lays a label out on the
advances `hmtx` declares. The eight figures refused 74 marks for being text and refuse 0. The door is
226 values and 250 types, from 217 and 247.

**A quadratic becomes a cubic exactly, so a drawn letter spends no tolerance of its own.** A `glyf`
outline is a quadratic B-spline and every path here is cubic, and the cubic with controls
P₀ + (2/3)(Q − P₀) and P₂ + (2/3)(Q − P₂) is the same curve as the quadratic with control Q. The two
agree to twelve places at eleven parameters over every segment of an `o`, which is a part in a
thousand of a unit on a font of 1,000 units to the em. The only error in a letter is the flattening
the triangulation already spends: all 202 glyphs with an outline reach the four edges their own
...
three pixels across and an edge pixel is where an exact coverage and four samples resolved to five
levels differ by construction. So `npm run gate:gpu` carries two floors, 0.97 for a figure with its
labels taken out and 0.96 with them drawn, and all eight figures pass both: `boolean` 98.51 per cent
within 8 of 255 against 99.46, `frame` 98.75 against 99.83, `matrix` 97.06 against 98.29, `portrait`
97.55 against 98.28, `rotate` 97.06 against 99.69, `solids` 96.61 against 98.68, `surface` 96.94
against 97.75, `tangent` 97.29 against 98.52.

**A recording is painted by whatever the caller hands the recorder, and it needs no page.**
`recordFigure` took `paintFrame` and now takes a `paint`, so `painterGpu` records off a card: the card
draws on its own canvas and `paintPixels` writes the pixels `pixelsGpu` reads back onto the encoder's
canvas with `putImageData`, since a WebGPU canvas cannot be drawn into a two-dimensional context and
a WebGL 2 one is not asked to preserve its drawing buffer. `npm run gate:record` recorded 8 files and
records 32, every one holding exactly the frames the walk counted: each figure through both painters,
in the page and again in a worker on `OffscreenCanvas` where `typeof document` is `undefined`. A file
recorded in the worker is the size of the one recorded in the page to within 0.176 per cent, and the
last frame's inked share is the same to a tenth of a point in all 16 pairs. The readback is the cost,
at 856 to 12,518 milliseconds off a card against 100 to 3,827 through the painter, measured against a
software WebGL 2 rather than against a card.

**2.9.0 is cut and unpublished, and its twelve done-criteria are verified line by line in the cut's
own commit.** `@altpsyche/engine` moves to 0.5.0 in both the peer range and the dev range, and four
of the six workarounds the GPU painter carried are gone: `gpuSurface` opens its renderer through
`openRenderer` rather than gathering the offering and choosing a backend itself, no mark is refused
for its opacity, the gate draws all eight figures on one canvas, and the baked GLSL stage negates
clip-space y. The door is 217 values and 247 types, unchanged, since this version adds no name and
takes none away.

**The move alone drew every figure upside down, and that was the version's real finding.** The WebGL
2 backend renders into a colour target of its own and blits it onto the canvas as of 0.5.0, which is
what lets it give a WGSL frame the top-left framebuffer origin that language counts rows from, and it
reads `framebufferOrigin` off the frame to decide whether to apply the flip. `glslFrameOf` stamps
`top-left` on everything it translates, because every stage the engine's own build bakes carries the
clip-space y negation. This package bakes its GLSL by hand and that stage left y alone, so it claimed
a negation it did not carry: `npm run gate:gpu` went from 8 of 8 figures agreeing to 1 of 8, with
`surface` at 61.00 per cent of its pixels within 8 of 255 against 97.51, and every disagreeing
figure's inked share up by about half, which is a picture drawn over its own mirror.

**Two of the six were not workarounds and the planning session is where that was read.** The gate has
never screenshotted a canvas: that reading describes the spike, and `gates/gpu.mjs` has read through
`FrameRenderer.frame` since it was written. And cutting a clip into the geometry places an edge a
scissor cannot: a scissor is whole pixels and the engine's validator refuses a rectangle that is not,
where `matrix`'s box runs from 94.2857 to 608.5714 pixels across and `surface`'s from 230.6250 to
474.3750, so rounding moves an edge by up to 0.4286 of a pixel.

**What the eight figures read now, against what they read at 2.8.0**, whole figure against whole
figure, since taking the refusal out puts the blended marks into the drawable list and that list is
not the same list either side: `tangent` 95.02 per cent within 8 to 98.52 with its worst channel 252
to 126, `portrait` 96.24 to 98.28, `matrix` 97.04 to 98.29, `surface` 97.51 to 97.75, and `boolean`,
`frame`, `rotate` and `solids` within 0.02 of where they were. 105 marks were refused across the
eight and 0 are now. The gate draws them in 251 milliseconds against 300, since one renderer holds
one program cache across every figure.

**The solid demo gained nothing at this version and that is a fact rather than a gap.** It carries no
mark under full opacity and no clip, so what it checks here is that 1,060 marks and 11,117 triangles
still agree with the sheet over 98.68 per cent of its pixels.
...
dynamic import, `paintFrame` gives each frame its ground, and `figureTime` says what a moment past
the end of a figure reads. The door is 208 values and 240 types.

**The bytes are gated in a browser and never in `npm test`, and that was the call this version turned
on.** Node 26.8.1 answers `[]` to `getEncodableVideoCodecs()` because it has no WebCodecs
`VideoEncoder`, and it has no canvas to rasterise onto either. So `npm run gate:record` serves this
repository to Chromium through `playwright` and records all eight committed figures, then reads each
file back in Node, since taking a container apart needs no encoder. Every file holds exactly the
frames the walk counted, 9.7 MB over the eight, and the last frame of each is between 3.7 and 81.9
per cent drawn.
...
- [ ] **3. A version is published from a tag. Waits on Siva, who deferred it on 2026-09-24.** It
      ...
      carries 3.0.0. 3.4.0 is cut, and 4.0.0 is cut after it. **npm carries 3.4.1 as `latest` since 2026-09-26**, published by hand, so the first tag is 4.0.0 or later.
...
- **The recorder's seam is written already, in the consumer, and 2.6.0 should take it rather than
  ...
  behind it satisfy the same seam at 2.10.0. **The walk has already drifted**: that file counts
