...
import { coordsOf, interval, pointOf, scaleOf, scaled, unscaled } from '../index.js';
...
describe('a scale', () => {
  it('puts each end of the graph at each end of the units', () => {
    expect(scaled(across, -1)).toBeCloseTo(-4.6, 12);
    expect(scaled(across, 4)).toBeCloseTo(4.6, 12);
  });

  it('reads a place back as a number on the axis', () => {
    expect(unscaled(across, -4.6)).toBeCloseTo(-1, 12);
    expect(unscaled(across, 4.6)).toBeCloseTo(4, 12);
    expect(unscaled(across, 0)).toBeCloseTo(1.5, 12);
  });

  it('carries a number past the end of the graph past the end of the units', () => {
    expect(scaled(across, 9)).toBeCloseTo(13.8, 12);
    expect(scaled(across, -6)).toBeCloseTo(-13.8, 12);
  });

  it('runs backwards where the units are given the other way round', () => {
    const down = scaleOf(interval(0, 9), interval(2.4, -2.4));
    expect(scaled(down, 0)).toBeCloseTo(2.4, 12);
    expect(scaled(down, 9)).toBeCloseTo(-2.4, 12);
  });
  ...
  it('round trips a thousand samples back to where they started', () => {
    ...
    for (const scale of [across, up]) {
      for (let step = 0; step <= 1000; step++) {
        ...
        worst = Math.max(worst, Math.abs(unscaled(scale, scaled(scale, value)) - value));
      }
    }
...
describe('a pair of scales', () => {
  ...
  it('scales each axis on its own, since the two spans differ', () => {
    ...
    expect(scaled(across, 1) - scaled(across, 0)).toBeCloseTo(1.84, 12);
    expect(scaled(up, 1) - scaled(up, 0)).toBeCloseTo(0.48, 12);
  });
});
